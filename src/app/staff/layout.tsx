...
const getSidebarGroups = (permissions: Permissions = {}) => {
  const groups = [
    {
      label: "Overview",
      items: [
  ...
  if (permissions.manageClients) {
    peopleItems.push({ label: "Clients", href: "/staff/clients", icon: Users });
  }
  ...
  if (permissions.manageAttendance) {
    academyItems.push({ label: "Attendance", href: "/staff/attendance", icon: CalendarCheck });
  }
  if (permissions.manageMatches) {
    academyItems.push({ label: "Matches", href: "/staff/matches", icon: Trophy });
  }
  academyItems.push({ label: "Player Records", href: "/staff/records", icon: BarChart3 });
  if (academyItems.length > 0) {
    groups.push({ label: "Academy", items: academyItems });
  }

  const businessItems = [];
  if (permissions.manageFees) {
    businessItems.push({ label: "Payments", href: "/staff/payments", icon: CreditCard });
  }
  if (permissions.manageEnquiries) {
    businessItems.push({ label: "Enquiries", href: "/staff/enquiries", icon: MessageSquare });
  }
...
function StaffLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const sidebarGroups = getSidebarGroups(permissions);
  ...
  if (!mounted || isLoading || !user || user.role !== "staff") {
    return (
      ...
      <aside className={cn(
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-8 border-b border-white/5 flex items-center justify-between">
            <Link href="/staff" className="flex items-center gap-3" onClick={() => { if (window.innerWidth < 1024) setSidebarOpen(false); }}>
              ...
              <div className="flex flex-col">
                <span className="font-black text-lg leading-tight tracking-tight uppercase">Staff</span>
          ...
          <div className="p-6 border-t border-white/5">
            <div className="flex items-center gap-4 mb-4 p-2 rounded-xl">
              <div className="w-10 h-10 rounded-full bg-academy-red flex items-center justify-center font-black">
                {user.name ? user.name.split(" ").map((n) => n[0]).join("").toUpperCase().slice(0, 2) : "ST"}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-black uppercase truncate">{user.name || "Staff"}</p>
                <p className="text-[10px] text-gray-500 font-bold uppercase truncate">Staff Member</p>
              </div>
            </div>
            <Button 
              variant="outline" 
